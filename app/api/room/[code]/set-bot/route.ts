...
import { setSeatBot } from "@/lib/room/rooms";
import { respondWithRoom } from "@/lib/room/api";
import { LEVELS } from "@/lib/engine/bots";

export async function POST(req: Request, ctx: RouteContext<"/api/room/[code]/set-bot">) {
  ...
  const result = await setSeatBot(code, token, seat, level as keyof typeof LEVELS);
